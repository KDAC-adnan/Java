				...
				case 7:
					try (FileOutputStream fout = new FileOutputStream("./Data/Books.txt")) {
						try (DataOutputStream dout = new DataOutputStream(fout)) {
							for (Book bookEle : library) {
								dout.writeUTF(bookEle.getIsbn());
								dout.writeDouble(bookEle.getPrice());
								dout.writeUTF(bookEle.getAuthorName());
								dout.writeInt(bookEle.getQuantity());
							}
						}
					} catch (Exception e) {
				...
				case 8: {
					try (FileInputStream fin = new FileInputStream("./Data/Books.txt")) {
						try (DataInputStream din = new DataInputStream(fin)) {
							while (true) {
								Book bookE = new Book();
								bookE.setIsbn(din.readUTF());
								bookE.setPrice(din.readDouble());
								bookE.setAuthorName(din.readUTF());
								bookE.setQuantity(din.readInt());
